...
public sealed class S1100Scanner
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            while (rawLinesReceived < expectedRawLines)
            {
                ...
                var rawBlock = await ReadScanBlockAsync(transport, profile, blockLines, cancellationToken).ConfigureAwait(false);
    ...
    private static async ValueTask<byte[]> ReadScanBlockAsync(
        IScannerTransport transport,
